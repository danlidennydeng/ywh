...
class ReasonsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create reason" do
    assert_difference('Reason.count') do
      post reasons_url, params: { reason: { edited_by: @reason.edited_by, name: @reason.name, note: @reason.note } }
    end
  ...
  test "should update reason" do
    patch reason_url(@reason), params: { reason: { edited_by: @reason.edited_by, name: @reason.name, note: @reason.note } }
